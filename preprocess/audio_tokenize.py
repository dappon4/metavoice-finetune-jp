...
def main(file_name, base_dir, num_shards, shard_id):
    ...
    if not os.path.exists(os.path.join(base_dir, "tok")):
        os.makedirs(os.path.join(base_dir, "tok"))
    print(start, end)
    for file_name in tqdm(files[start:end]):
        in_file = os.path.join(base_dir,file_name)
        out_file = os.path.join(base_dir, file_name).replace("wav", "tok")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_file", default="../datasets/tiny.tsv")
