...
def main(file_name, base_dir, num_shards, shard_id):
    files = pd.read_csv(file_name, sep="\t", header=None, names=["path", "text"])["path"].tolist()
    start, end = get_ids(len(files), num_shards, shard_id)
    print(start, end)
    if not os.path.exists(os.path.join(base_dir, "emb")):
        os.makedirs(os.path.join(base_dir, "emb"))
    for file_name in tqdm(files[start:end]):
        in_file = os.path.join(base_dir,file_name)
        out_file = os.path.join(base_dir, file_name).replace("wav", "emb")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_file", default="../datasets/tiny.tsv", help="Name of the file")
    parser.add_argument("--base_dir", type=str, default="../datasets", help="base_dir")
